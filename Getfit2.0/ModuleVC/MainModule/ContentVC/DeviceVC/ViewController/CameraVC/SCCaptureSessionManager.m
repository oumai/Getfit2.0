...
//        [SCCommon saveImageToPhotoAlbum:image];
        ...
        CGSize size = CGSizeMake(SC_APP_SIZE.width * 2, SC_APP_SIZE.height * 2);
...
//        CGRect cropFrame = CGRectMake((scaledImage.size.width - size.width) / 2, (scaledImage.size.height - size.height) / 2 + headHeight, size.width, size.height);
        CGRect cropFrame = CGRectMake(0,0, size.width, size.height);
...
- (void)switchFlashMode:(UIButton*)sender {
    ...
    if (!captureDeviceClass) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"提示信息", nil) message:NSLocalizedString(@"您的设备没有拍照功能", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"Sure", nil) otherButtonTitles: nil];
        [alert show];
        return;
    }
...
//        }
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"提示信息", nil) message:NSLocalizedString(@"您的设备没有闪光灯功能", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"噢T_T", nil) otherButtonTitles: nil];
        [alert show];
    }
