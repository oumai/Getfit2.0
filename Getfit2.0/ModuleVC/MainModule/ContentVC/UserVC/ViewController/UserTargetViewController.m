...
@interface UserTargetViewController ()

@end

@implementation UserTargetViewController
...
- (void)loadNavBar
{
    self.tipTitle.text = KK_Text(@"Daily Target");
}
...
{
    NSLog(@"保存>>>>>>>>>>");
    [[UserInfoHelper sharedInstance] updateUserSportTargetAndSleepTarget:YES];
    [self performSelector:@selector(leftBarButton) withObject:nil afterDelay:2]; //2秒之后返回
}
...
- (void)loadScrollView
{
    CGFloat screenHeight = FitScreenNumber(33 - 64, 0 - 64, 0 - 64, 0 - 64, 0 - 64);
    
    UILabel *stepLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 80 + 40, 30)];
    ...
    UILabel *stepUnitLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 200, 30)];
    ...
    _stepTargetLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 160, 30)];
    ...
    CGFloat screenSleepHeight = FitScreenNumber(60 - 64, 0 - 64, 0 - 64, 0 - 64, 0 - 64);
    
    UIView * line = [[UIView alloc] initWithFrame:CGRectMake(0, 230 - screenHeight, Maxwidth, 1)];
    line.backgroundColor = UIColorHEX(0x888b90);
    [self.view addSubview:line];
    
    UILabel *sleepLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 200, 30)];
    ...
    _stepScrollView.contentSize = CGSizeMake(3350, 96);//宽，决定滚多远
    ...
    // 开始目标
    NSLog(@"睡眠目标>>>>>%ld",(long)[UserInfoHelper sharedInstance].userModel.targetSleep );
    NSLog(@"step目标>>>>>%ld",(long)[UserInfoHelper sharedInstance].userModel.targetSteps);
    
    CGFloat ContentOffsetX = ([UserInfoHelper sharedInstance].userModel.targetSteps - 1000) / 10 ;
    [_stepScrollView setContentOffset:CGPointMake(ContentOffsetX, 0)];
    ...
    _sleepScrollView.frame = CGRectMake(0, 304 - screenSleepHeight, Maxwidth ,96);
    _sleepScrollView.backgroundColor = [UIColor clearColor];
    [self.view addSubview:_sleepScrollView];
    _sleepScrollView.contentSize = CGSizeMake(1030, 96);
    ...
    CGFloat sleepContentOffsetX = [UserInfoHelper sharedInstance].userModel.targetSleep ;
    [_sleepScrollView setContentOffset:CGPointMake(sleepContentOffsetX, 0)];
...
- (void)sleepDelayOffSet
{
    ...
    int sleep = _sleepScrollView.contentOffset.x / 10 *10 +120;
    [UserInfoHelper sharedInstance].userModel.targetSleep = sleep - 120;
    _hourLabel.text = [NSString stringWithFormat:@"%d",sleep/60];
    _minLabel.text = [NSString stringWithFormat:@"%d",sleep%60];
    if (sleep/60 == 12) {
        _minLabel.text = @"0";
    }
    if (sleep%60%10!=0) {
        _minLabel.text = [NSString stringWithFormat:@"%.0f",floor(sleep%60/10)*10];
    }
}
