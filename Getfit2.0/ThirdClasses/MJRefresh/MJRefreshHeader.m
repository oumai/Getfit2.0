...
- (void)loadTitleLabel
{
    ...
    [_updatedTimeLabel setHidden:YES];
    
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(Maxwidth/2, 24, 100, 20)];
...
- (void)setUpdatedTime:(NSDate *)updatedTime
{
    ...
    if (updatedTime) {
        ...
        // 3.显示日期
        self.updatedTimeLabel.text = [NSString stringWithFormat:@"Last updated：%@", time];
    } else {
        self.updatedTimeLabel.text = @"Last updated：No record";
    }
}
